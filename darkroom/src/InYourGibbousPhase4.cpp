...
namespace InYourGibbousPhase4 {
    ...
    PoseEstimator::PoseEstimator(int numberOfSensors) :
            Functor<double>(6+6, 2 * numberOfSensors), numberOfSensors(numberOfSensors) {
    }
    ...
    int PoseEstimator::operator()(const VectorXd &x, VectorXd &fvec) const {
        ...
        for(int i=0; i<numberOfSensors;i++) {
            double azimuth_calib = azimuths[i];
            double elevation_calib = elevations[i];
            double temp_azimuth_calib1 = x(curve_horizontal) * pow(-sin(azimuth_calib) * cos(elevation_calib), 2.0);
            double temp_azimuth_calib2 = x(gibmag_horizontal) *cos(azimuth_calib + x(gibphase_horizontal));
            double temp_elevation_calib1 = x(curve_vertical) * pow(cos(azimuth_calib) * sin(elevation_calib), 2.0);
            double temp_elevation_calib2 = x(gibmag_vertical) *cos(elevation_calib + x(gibphase_vertical));
            azimuth_calib += (temp_azimuth_calib1);
            elevation_calib += (temp_elevation_calib1);


            double u = tan(M_PI_2-azimuth_calib), v = tan(elevation_calib-M_PI_2);
